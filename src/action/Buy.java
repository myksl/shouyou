...
import factory.BuyOrderFactory;
import factory.ProductFactory;
import factory.SellOrderFactory;
import vo.BuyOrder;
import vo.Product;
import vo.SellOrder;
...
public class Buy {
	...
	public String buy(){
		...
		try {
			product = ProductFactory.get().findById(productListIndex);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		...
		sellOrder.setOrderName(product.getProductName());
		...
		buyOrder.setOrderName(product.getProductName());
		...
		try {
			flag = BuyOrderFactory.get().doCreate(buyOrder);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(!flag){
		...
		try {
			flag = SellOrderFactory.get().doCreate(sellOrder);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(!flag){
